...
export default function GisAdminOverviewPage() {
  const router = useRouter();

  const { data: metrics, isLoading: metricsLoading } = useQuery({
    queryKey: ["gis-admin-overview"],
    queryFn: () => api.get<{ metrics: AgencyAdminMetrics }>("/gis/admin/overview").then((r) => r.data.metrics),
    refetchInterval: 60_000,
  });

  const { data: topApplicants } = useQuery({
    queryKey: ["gis-admin-applicants", 1],
    queryFn: () => api.get<PaginatedResponse<AgencyApplicantSummary>>("/gis/admin/applicants", { params: { per_page: 5 } }).then((r) => r.data),
  });

  const { data: topOfficers } = useQuery({
    queryKey: ["gis-admin-officers", 1],
    queryFn: () => api.get<PaginatedResponse<AgencyOfficerSummary>>("/gis/admin/officers", { params: { per_page: 5 } }).then((r) => r.data),
  });

  const { data: recentApplications } = useQuery({
    queryKey: ["gis-admin-applications", 1],
    queryFn: () =>
      api
  ...
  return (
    <DashboardShell
      title="GIS Admin Overview"
      description="Agency-level analytics, applicants, and officer management"
      actions={
        <Button leftIcon={<BarChart3 size={16} />} onClick={() => router.push("/dashboard/gis/cases")}>
          Case Operations
        </Button>
      }
    >
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-border shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              ...
              <p className="text-xs text-text-muted">Latest GIS applications across all queues</p>
            </div>
            <Button size="sm" variant="secondary" onClick={() => router.push("/dashboard/gis/admin/applications")}>
            ...
            {!recentApplications?.data?.length ? (
              ...
              recentApplications.data.map((application) => (
                <button
                  ...
                  onClick={() => router.push(`/dashboard/gis/cases/${application.id}`)}
                >
                  <div>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              ...
              <p className="text-xs text-text-muted">Queues with approaching deadlines</p>
            </div>
            <Button size="sm" variant="secondary" onClick={() => router.push("/dashboard/gis/sla-alerts")}>
              Review Alerts
            </Button>
          </div>
            ...
            <div className="p-4 rounded-xl bg-danger/5 border border-danger/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">Ensure SLAs are met</p>
                </div>
              </div>
            </div>
            <div className="p-4 rounded-xl bg-warning/5 border border-warning/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">Prioritize ageing cases</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-border shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              <h2 className="text-base font-bold text-text-primary">Top Applicants</h2>
              <p className="text-xs text-text-muted">Applicants with active GIS cases</p>
            </div>
            <Button size="sm" variant="secondary" onClick={() => router.push("/dashboard/gis/admin/applicants")}>
              Manage
            </Button>
          </div>
              ...
              {topApplicants.data.map((applicant) => (
                <div key={applicant.id} className="px-6 py-4">
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    {applicant.gis_applications_count ?? 0} applications
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              ...
              <p className="text-xs text-text-muted">Active GIS reviewers & approvers</p>
            </div>
            <Button size="sm" variant="secondary" onClick={() => router.push("/dashboard/gis/admin/officers")}>
              Manage
            </Button>
          </div>
